...
public class ComposeBlockRepetitionFilter extends AbstractComposeBlockFilter {
	...
	Map<Double, Integer> getRepetitions( List<MusicBlock> musicBlocksToCheck ) {
		...
		// Repetition can consits from 1 to N music blocks. Iterating through them trying to count number of repetitions
		for ( int blocksInRepetition = 1; blocksInRepetition < musicBlocksToCheck.size(); blocksInRepetition++ ) {
			repetition.add( musicBlocksToCheck.get( blocksInRepetition ) );
			musicBlocks.remove( blocksInRepetition );
			if ( repetition.stream().mapToDouble( MusicBlock::getRhythmValue ).sum() < 2 * WHOLE_NOTE ) {
				...
				// Also number of comarison when cycle will break will show us number of repetitions
				for ( int numberOfComparison = 0; numberOfComparison < musicBlocks.size() / repetition.size(); numberOfComparison++ ) {
					// In each comparison we need to compare all blocks in the repetition with correcponding blocks in musicBlock list
					for ( int numberOfBlockInRepetition = 0; numberOfBlockInRepetition < repetition.size(); numberOfBlockInRepetition++ ) {
						if ( !equality( repetition.get( numberOfBlockInRepetition ), musicBlocks.get( musicBlocks.size() - ( numberOfComparison + 1 ) * repetition.size() + numberOfBlockInRepetition ) ) ) {
	...
	private boolean equality( MusicBlock musicBlock, MusicBlock musicBlock1 ) {
		// TODO Implementation
		return false;
	}
