...
public class MelodyMetricEqualityAnalyzer implements EqualityMetricAnalyzer<InstrumentPart> {
   ...
   * to be like first.
   * Firstly we count how many cutting and joining should be performed to make second melody rhythmic identical to
   * first
  ...
  public double getEqualityMetric(InstrumentPart firstInstrumentPart, InstrumentPart secondInstrumentPart) {
    ...
    List<Double> unionRhythmValues =
        getUnionRhythmValues(firstInstrumentPart.getNoteGroups(), secondInstrumentPart.getNoteGroups());
    List<NoteGroup> transformedFirst =
     ...
     * some notes
     * and joining some.
     * numberOfCuttedNotes is the number of notes that should be cutted in firstInstrumentPart to get it
     * unionRhythm-like
     * this is exact number of cuttings that should be performed on secondInstrumentPart to get it
     * firstInstrumentPart rhythmic like
    ...
    List<Boolean> isStrongTime = getStrongTimeMetric(unionRhythmValues);
		...
		Taking into consideration that it should be done regardless transposition we will count several metrics and
		choose min
		We are going to use brute force - counting metric considering transposition of first note of
		secondInstrumentPart
  ...
  List<Double> getUnionRhythmValues(List<NoteGroup> firstNotes, List<NoteGroup> secondNotes) {
    Set<Double> firstEdges = new HashSet<>(RecombineUtils.getRhythmEdgeList(firstNotes));
    List<Double> secondEdges = RecombineUtils.getRhythmEdgeList(secondNotes);
