...
public class CompositionSlicerTest {
	...
	public void restInTheBeginningTestSlice() {
		...
		Phrase phraseToSlice = composition.getPart( 3 ).getPhrase( 0 );

		View.notate( phraseToSlice );
		suspend();

		List< List< Note> > sliceToTest = compositionSlicer.slice( phraseToSlice, WHOLE_NOTE );
//		assertEquals( sliceToTest.size(), 3 );
