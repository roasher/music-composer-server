package decomposer.analyzer.form.rhythm_old.equality;

import java.util.List;
import java.util.Map;

/**
 * Class examines the equality between "List of counts of Rhythm Entries"
...
public class RhythmEqualityTest {
	...
	/**
	 * There can be rhythm_old values in one phrase that aren't present in the other. This var declares max number of sutch
	 * values that we still considering that phrases CAN be rhythmically equals
	 */
	private int maxUniqueRhythmValuesCount;
	/**
	 * Declares max percent deviation between number of first phrase rhythm_old value and number of second phrase same rhythm_old value
	 */
	private double maxRhythmPercentageEntryDeviation;

	public boolean isEqual( List<Map<Double, Double>> firstListOfCountsOfRhythmEntries, List<Map<Double, Double>> secondListOfCountsOfRhythmEntries ) {
		...
		for ( int currentInstrument = 0; currentInstrument < firstListOfCountsOfRhythmEntries.size(); currentInstrument++ ) {
			if ( isCountsOfRhythmEntriesEquals( firstListOfCountsOfRhythmEntries.get( currentInstrument ), secondListOfCountsOfRhythmEntries.get( currentInstrument ) ) ) {
				numberOfEqualParts++;
			}
		}
	...
	public boolean isCountsOfRhythmEntriesEquals( Map<Double, Double> firstCountOfRhythmEntryMap, Map<Double, Double> secontCountOfRhythmEntryMap ) {
		...
		for ( Map.Entry<Double, Double> entry : firstCountOfRhythmEntryMap.entrySet() ) {
			...
			if ( secondCountRhythmEntry != null ) {
				if ( Math.abs( secondCountRhythmEntry.doubleValue() - entry.getValue().doubleValue() ) > maxRhythmPercentageEntryDeviation ) {
					return false;
				}
			} else {
