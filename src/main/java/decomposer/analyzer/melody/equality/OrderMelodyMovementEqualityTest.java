...
public class OrderMelodyMovementEqualityTest extends AbstractMelodyMovementEqualityTest {
    ...
    private boolean testIfEqualsToSwappedInterval( final MelodyMovement etalonMelodyMovement, MelodyMovement melodyMovementToSwap, int numberOfIntervalToSwap, int numberOfSwappedIntervals ) {
        ...
        boolean equalsIfRemainInterval = testIfEqualsToSwappedInterval( etalonMelodyMovement, melodyMovementToSwap, numberOfIntervalToSwap + 1, numberOfSwappedIntervals );
        ...
        if ( numberOfSwappedIntervals <= maxNumberOfIntervalsHavingSwappedNotes )
        {
            ...
            MelodyMovement melodyMovementWithSwappedIntervals = melodyMovementToSwap.createMelodyMovementWithSwappedNotes( numberOfIntervalToSwap );
            if ( etalonMelodyMovement.equals( melodyMovementWithSwappedIntervals ) ) {
                return true;
            } else {
                equalsIfChangeInterval = testIfEqualsToSwappedInterval( etalonMelodyMovement, melodyMovementWithSwappedIntervals, numberOfIntervalToSwap + 1, numberOfSwappedIntervals );
            }
        } else {
            return false;
        }
